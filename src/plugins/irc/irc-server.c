                                       ...
                                       int argc, char **argv)
{
    ...
    for (i = 0; i < argc; i++)
    {
        ...
        if (option_name)
        {
            ...
            if (index_option >= 0)
            {
                if (weechat_config_option_set (
                        server->options[index_option],
                        ptr_value, 1) == WEECHAT_CONFIG_OPTION_SET_ERROR)
                {
                    weechat_printf (
                        NULL,
                        _("%s%s: warning: unable to set option \"%s\" for server \"%s\""),
                        weechat_prefix ("error"), IRC_PLUGIN_NAME,
                        argv[i], server->name);
                }
            }
            else
            {
                weechat_printf (
                    NULL,
                    _("%s%s: warning: unknown option \"%s\" for server \"%s\""),
                    weechat_prefix ("error"), IRC_PLUGIN_NAME,
                    argv[i], server->name);
            }
            free (option_name);
